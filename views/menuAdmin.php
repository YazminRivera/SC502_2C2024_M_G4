<link rel="stylesheet" href="./assets/css/menu.css">
<div class="seccion1">
  <div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center">
      <div class="d-flex align-items-center">
        <a href="index.php"><img src="https://cdn-icons-png.flaticon.com/512/5871/5871586.png" alt="Logo" style="height: 80px;"></a>
        <h1 style="color: white; margin-left: 10px;">Hogar de Adopción y Rescate</h1>
      </div>
      <div>
        
        <a href="indexadm.php" class="btn btnInicio">Administrador</a>
        <a href="./cerraSesion.php" class="btn btnInicio">Cerrar Sesion</a>
      </div>
    </div>
  </div>
  <nav class="navbar navbar-expand-lg bottom-bar">
  <div class="container-fluid">
    <div class="collapse navbar-collapse">
      <ul class="navbar-nav">
        <li class="nav-item">
          <a class="nav-link" href="index.php">Inicio</a>
        </li>
        <li class="nav-item">
        <li class="nav-item">
          <a class="nav-link" href="verAnimales.php">Animales Registrados</a>
        </li>
          <a class="nav-link" href="encontrados.php">Encontrados</a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="desapariciones.php">Desapariciones</a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="adopciones.php">Adopciones</a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="infoCasAdm.php">Información Castraciones</a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="solicitud_apoyo.php">Solicitar Apoyo</a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="listarSolicitudes.php">Solicitudes Apoyo</a>
        </li>
      </ul>
    </div>
  </div>
</nav>
</div>
